...
# ------------------------------------------------------------
# 10. Живые услуги – общая схема, но поля те же, что у соответствующих типов
# ------------------------------------------------------------
# Для живых услуг используем те же схемы, что и для обычных, но с дополнительным пояснением.
# В коде обработки они будут отфильтрованы по подкатегории live_*, а схема возьмётся из родительской категории.
# Поэтому отдельные схемы для живых не нужны.
...
def get_schema_for_service(service_id: int, service_name: str) -> ServiceSchema | None:
    ...
    if "старты бота" in lower_name:
        ...
        return SCHEMAS["старты бota"]

    # Рефералы (особый случай, но можно отнести к стартам)
    ...
    # Для живых услуг (наследуем схему по типу, но здесь не требуется, так как фильтр по подкатегории
    # и определение схемы будет через другие ключевые слова)
    return None