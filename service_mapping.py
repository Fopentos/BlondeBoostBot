...
# 1d. views_private — просмотры для закрытых каналов (бот @privateviewss_bot)
...
# 1i. views_premium — Premium просмотры
...
# 2b. reactions_private — реакции для закрытых каналов (аккаунты уже подписаны)
...
# 2e. reactions_plus — реакции + просмотры (комбо-услуги)
_REACTIONS_PLUS = frozenset({
    4752, 4753,
...
# 2g. reactions_stories — реакции на истории
...
_SUBS_NORMAL = frozenset({
    ...
    3422, 3423, 3456, 3457, 3459, 3465, 3466, 3467,
...
# 3d. subs_live — живые подписчики с рекламы
_SUBS_LIVE = frozenset({
    3948, 3949, 3950, 3951, 3952, 3953, 3954, 3955, 3956,
})

# 3e. subs_premium — Premium подписчики
...
_BOOSTS_OPEN = frozenset({
    1269, 1270, 2970, 3148, 3911, 3912, 4255, 4328, 4556, 4575, 4654, 4756,
})
...
SUBCATEGORY_SERVICE_IDS: dict[str, frozenset[int]] = {
    ...
    "reposts_premium": _REPOSTS_PREMIUM,
    # Live
