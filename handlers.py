...
async def service_selected(callback: CallbackQuery, state: FSMContext):
    ...
    # BUG FIX: store api_description as plain text — never embed raw API text in Markdown
    ...
    if schema:
        ...
        await state.update_data(price_rub_per_1000=price_rub_per_1000)
        # BUG FIX: send api_description as plain text (no parse_mode) to avoid Markdown crashes
        if api_description:
            await callback.message.answer(
                t(lang, "api_instruction_header") + api_description
            )
        await callback.message.edit_text(
            t(lang, "service_info",
              name=service["name"],
              price=price_rub_per_1000,
              min=service["min"],
              max=service["max"]),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=back_button(lang)
...
async def get_quantity(message: Message, state: FSMContext):
    ...
    await message.answer(
        t(lang, "order_confirm",
          service=service["name"], link=data["link"],
          quantity=quantity, total=total_rub),
        parse_mode=ParseMode.MARKDOWN,
