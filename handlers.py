...
from service_mapping import SUBCATEGORY_SERVICE_IDS
...
SUBCATEGORY_INSTRUCTIONS = {
    ...
    "views_premium": (
        "Premium просмотры добавляются с аккаунтов Telegram Premium.\n\n"
        "Нужна ссылка на ПОСТ (не на канал).\n"
        "Пример: https://t.me/durov/123\n\n"
        "Для просмотров на несколько постов — укажите ссылку на КАНАЛ.\n"
        "Сколько постов охватывается — указано в названии услуги.\n\n"
        "Старт: обычно в течение 1–24 часов."
    ),
    ...
    "reactions_stories": (
        "Реакции добавятся на историю (Story) в Telegram.\n\n"
        "Как найти ссылку на историю:\n"
        "1. Откройте историю\n"
        "2. Нажмите «...» (три точки) → «Поделиться» → «Скопировать ссылку»\n\n"
        "Формат ссылки: https://t.me/username/s/НОМЕР\n"
        "Пример: https://t.me/durov/s/5\n\n"
        "История должна быть активной. Эмодзи реакции указан в названии услуги."
    ),
    ...
    "subs_premium": (
        "Premium подписчики — аккаунты с активной подпиской Telegram Premium.\n\n"
        "Для ОТКРЫТОГО канала: https://t.me/username\n"
        "Для ЗАКРЫТОГО канала: пригласительная ссылка https://t.me/+XXXXXX\n\n"
        "Premium аккаунтов меньше, чем обычных — скорость может быть ниже.\n"
    ...
    "reposts_premium": (
        "Ваш пост будет пересылаться аккаунтами Telegram Premium.\n\n"
        "Нужна ссылка на ПОСТ.\n"
        "Пример: https://t.me/durov/123\n\n"
        "Как найти: откройте пост → «Поделиться» → «Скопировать ссылку».\n\n"
        "Premium-репосты выглядят более авторитетно в Telegram Analytics."
    ),
...
def _get_services_for_subcategory(sub: str) -> list:
    """Return filtered services for a subcategory using explicit ID mapping."""
    service_ids = SUBCATEGORY_SERVICE_IDS.get(sub, frozenset())
    return [s for s in services_cache if s["service"] in service_ids]


@router.callback_query(F.data.startswith("sub_"))
async def handle_subcategory(callback: CallbackQuery, state: FSMContext):
    sub = callback.data.split("_", 1)[1]
    if sub not in SUBCATEGORY_SERVICE_IDS:
        await callback.answer("Подкатегория не найдена")
        return
    filtered = _get_services_for_subcategory(sub)
    if not filtered:
...
async def redirect_to_subscribe(callback: CallbackQuery, state: FSMContext):
    filtered = _get_services_for_subcategory("reactions_subscribe")
    if not filtered:
        try:
            await callback.message.edit_text("В этой подкатегории пока нет услуг.", reply_markup=back_button())
        except TelegramBadRequest:
