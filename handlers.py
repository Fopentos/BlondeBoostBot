...
from keyboards import (
    ...
    reposts_submenu, premium_submenu
)
...
from service_schemas import get_schema_for_service, FIELD_LINK, FIELD_QUANTITY, FIELD_POSTS_COUNT, FIELD_SPEED, FIELD_VOTE_OPTION, FIELD_COMMENT_TEXT
...
# ---------------------- Фильтры для подкатегорий ----------------------
SUBCATEGORY_FILTERS = {
    # Просмотры
    "views_single": lambda s: "просмотры на пост" in s["name"].lower() and not any(x in s["name"].lower() for x in ["последних", "будущих", "в минуту", "закрытых каналов", "умные", "авто"]),
    "views_multi": lambda s: "последних постов" in s["name"].lower(),
    ...
    # Реакции
    "reactions_normal": lambda s: "реакц" in s["name"].lower() and "закрытых каналов" not in s["name"].lower() and "премиум" not in s["name"].lower() and "+ просмотры" not in s["name"].lower(),
    "reactions_private": lambda s: "реакц" in s["name"].lower() and "закрытых каналов" in s["name"].lower(),
    "reactions_premium": lambda s: "реакц" in s["name"].lower() and ("премиум" in s["name"].lower() or "premium" in s["name"].lower()),
    "reactions_plus": lambda s: "реакц" in s["name"].lower() and "+ просмотры" in s["name"].lower(),
    # Подписчики
    "subs_normal": lambda s: "подписчик" in s["name"].lower() and not any(x in s["name"].lower() for x in ["авто просмотры", "24/7", "живые", "премиум"]),
    ...
    # Старты бота
    "starts_normal": lambda s: "старты бота" in s["name"].lower() and "активность" not in s["name"].lower() and "премиум" not in s["name"].lower(),
    ...
    "reposts_geo": lambda s: "репост" in s["name"].lower() and any(country in s["name"] for country in ["Италия", "Германия", "Турция", "Израиль", "Индонезия", "Китай", "США", "Россия", "Индия", "Арабские"]),
    # Premium услуги
    "premium_subs": lambda s: "премиум подписчик" in s["name"].lower(),
    "premium_views": lambda s: "премиум просмотр" in s["name"].lower(),
    "premium_starts": lambda s: "премиум старты бота" in s["name"].lower(),
    "premium_reactions": lambda s: "премиум реакц" in s["name"].lower(),
    "premium_reposts": lambda s: "премиум репост" in s["name"].lower(),
}

# ---------------------- Вспомогательные функции для динамического заказа ----------------------
async def ask_next_field(message: Message, state: FSMContext):
    """Запрашивает следующее поле у пользователя, показывая инструкции"""
    ...
    if field["type"] == "days":
        # Создаём кнопки для выбора длительности
...
async def validate_field(field, value: str, service_name: str) -> tuple[bool, any, str]:
    """Валидация значения поля, возвращает (ok, converted_value, error_message)"""
    if field["type"] == FIELD_LINK:
        ...
        if "закрытых каналов" in service_name.lower() or "private" in service_name.lower():
            if "/c/" not in value:
...
async def create_dynamic_order(message: Message, state: FSMContext):
    """Создаёт заказ после сбора всех полей"""
    data = await state.get_data()
...
async def show_category(callback: CallbackQuery, state: FSMContext):
    ...
    # Показываем подменю в зависимости от категории
    if cat == "views":
        ...
        await callback.message.edit_text("⭐️ Выберите тип Premium услуг:", reply_markup=premium_submenu())
    else:
...
async def handle_subcategory(callback: CallbackQuery, state: FSMContext):
    sub = callback.data.split("_", 1)[1]  # например "views_single"
    if sub not in SUBCATEGORY_FILTERS:
        await callback.answer("Подкатегория не найдена")
        return
    filter_func = SUBCATEGORY_FILTERS[sub]
    # Фильтруем услуги, ограничивая сетью Telegram (или можно оставить все, но обычно Telegram)
    filtered_services = [s for s in services_cache if filter_func(s) and s.get("network") in ("Telegram", "Telegram Premium")]
    if not filtered_services:
        await callback.message.edit_text("В этой подкатегории пока нет услуг.", reply_markup=back_button())
        return
    # Сортируем по возрастанию цены по умолчанию
    filtered_services.sort(key=lambda s: float(s["rate"]))
    await state.update_data(category_services=filtered_services, category_name=sub, page=1, sort_order="asc")
    await send_category_page(callback.message, sub, filtered_services, 1, state)
    await callback.answer()
...
async def send_category_page(message, cat, services, page, state):
    ...
    # Кнопки сортировки
    data = await state.get_data()
    current_sort = data.get("sort_order", "asc")
    sort_buttons = [
        InlineKeyboardButton(text="💰 По возрастанию" + (" ✅" if current_sort == "asc" else ""), callback_data=f"sort_{cat}_asc"),
        InlineKeyboardButton(text="💰 По убыванию" + (" ✅" if current_sort == "desc" else ""), callback_data=f"sort_{cat}_desc"),
...
async def sort_services(callback: CallbackQuery, state: FSMContext):
    _, cat, order = callback.data.split("_")
    data = await state.get_data()
    services = data.get("category_services", [])
    if not services:
        await callback.answer("Ошибка, попробуйте снова")
        return
    # Сортировка по цене (rate)
    services_sorted = sorted(services, key=lambda s: float(s["rate"]), reverse=(order == "desc"))
...
async def start_dynamic_order(message, state, schema):
    ...
    # Показываем предупреждение/инструкцию, если есть
    if "pre_check" in schema:
        await message.answer(schema["pre_check"], parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Понятно, продолжить", callback_data="continue_dynamic")]]))
...
async def get_link(message: Message, state: FSMContext):
    ...
    if not link.startswith(("https://t.me/", "http://t.me/")):
        await message.answer("❌ Ссылка должна начинаться с https://t.me/ . Попробуйте снова.")
        return
    # Для закрытых каналов (если в названии есть ключевые слова)
    if "закрытых каналов" in service_name.lower() or "private" in service_name.lower():
        if "/c/" not in link:
            await message.answer("❌ Для закрытого канала ссылка должна быть в формате https://t.me/c/123456789/301. Попробуйте снова.")
            return
...
# ---------------------- Админ-панель (упрощённо, основное в admin.py) ----------------------
