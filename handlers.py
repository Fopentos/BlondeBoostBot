...
# Displayed as plain text (no Markdown) — safe for any service name/emoji.
...
async def validate_field(field, value: str, service_name: str):
    if field["type"] == FIELD_LINK:
        ...
        # Invite-link subcategories — must have t.me/+
        ...
        # Private-post subcategories — must have /c/
...
async def profile(callback: CallbackQuery, state: FSMContext):
    ...
    user = get_user(callback.from_user.id)
    balance = user["rub_balance"] if user else 0.0
    # safe_md prevents crash when full_name contains _ or *
    text = (
        f"👤 *Ваш профиль*\n\n"
...
async def show_history(callback: CallbackQuery, state: FSMContext):
    ...
    if not orders:
        ...
        for o in orders:
            # safe_md: service names from DB may contain _ or [
...
async def service_selected(callback: CallbackQuery, state: FSMContext):
    # Read category_name BEFORE state.clear() — it tells us which subcategory
    # the user was in, giving 100% accurate schema regardless of service name.
    ...
    # api_description stored for later display (shown as plain text — no Markdown)
    api_description = service.get("description", "").strip()

    # Schema lookup: subcategory key first (exact), then name heuristic, then fallback
...
async def start_dynamic_order(message, state, schema, category_name: str = "", api_description: str = ""):
    ...
    # Build instruction text — completely plain (no Markdown, no parse_mode).
    # Sources (in priority order):
    #   1. Our subcategory instruction (explains link format, prerequisites, etc.)
    #   2. API description from Twiboost (raw text, may contain _, [, emoji)
    parts = []
    ...
    if parts:
        instruction_text = "\n\n".join(parts)
        # Extra button for reactions_private (needs subscribers first)
        extra_buttons = []
...
async def get_quantity(message: Message, state: FSMContext):
    ...
    # Plain text confirmation — no parse_mode to avoid crashes on service names with _
    await message.answer(
