...
# ---------------------- Фильтры для подкатегорий (итоговые) ----------------------
SUBCATEGORY_FILTERS = {
    # Просмотры
    "views_single": lambda s: ("просмотры на пост" in s["name"].lower() or "просмотры [" in s["name"].lower()) and not any(x in s["name"].lower() for x in ["последних", "будущих", "в минуту", "закрытых", "умные", "авто", "telesco", "историю", "кружочки"]),
    ...
    # Реакции
    "reactions_normal": lambda s: ("реакц" in s["name"].lower() or "реакция" in s["name"].lower()) and "закрытых каналов" not in s["name"].lower() and "премиум" not in s["name"].lower() and "premium" not in s["name"].lower() and "+ просмотры" not in s["name"].lower(),
    ...
    "reactions_auto": lambda s: "авто реакции" in s["name"].lower(),
    # Подписчики
    "subs_normal": lambda s: "подписчик" in s["name"].lower() and not any(x in s["name"].lower() for x in ["авто просмотры", "24/7", "живые", "премиум", "premium"]),
...
async def dynamic_field_days(callback: CallbackQuery, state: FSMContext):
    ...
    fields = data["schema"]["fields"]
    if idx >= len(fields):
        await callback.answer("Ошибка", cache_time=5)
        return
    collected = data.get("collected_fields", {})
...
async def handle_subcategory(callback: CallbackQuery, state: FSMContext):
    ...
    if sub not in SUBCATEGORY_FILTERS:
        await callback.answer("Подкатегория не найдена", cache_time=5)
        return
...
async def send_category_page(message, cat, services, page, state):
    ...
    nav = []
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="◀️ Назад", callback_data=f"page_{cat}_{current_page-1}"))
    if current_page < total_pages:
        nav.append(InlineKeyboardButton(text="Вперёд ▶️", callback_data=f"page_{cat}_{current_page+1}"))
    if nav:
        buttons.append(nav)
    data = await state.get_data()
    sort_order = data.get("sort_order", "asc")
    sort_buttons = [
        InlineKeyboardButton(text="💰 По возрастанию" + (" ✅" if sort_order == "asc" else ""), callback_data=f"sort_{cat}_asc"),
        InlineKeyboardButton(text="💰 По убыванию" + (" ✅" if sort_order == "desc" else ""), callback_data=f"sort_{cat}_desc"),
...
async def paginate_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("processing"):
        await callback.answer("Подождите...", cache_time=5)
        return
...
async def service_selected(callback: CallbackQuery, state: FSMContext):
    ...
    service = next((s for s in services_cache if s["service"] == service_id), None)
    if not service:
        await callback.answer("Услуга не найдена", cache_time=5)
        return
...
async def get_link(message: Message, state: FSMContext):
    ...
    if not link.startswith(("https://t.me/", "http://t.me/")):
        ...
        return
    if "закрытых каналов" in service_name.lower() and "/c/" not in link:
