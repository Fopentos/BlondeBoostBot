...
from keyboards import (
    ...
    reposts_submenu, live_submenu, language_keyboard
)
...
from service_schemas import (
    ...
    FIELD_LINK, FIELD_QUANTITY, FIELD_POSTS_COUNT, FIELD_VOTE_OPTION, FIELD_COMMENT_TEXT
)
from locales import t


def safe_md(text: str) -> str:
    """Escape Markdown v1 special characters in dynamic user/API data."""
    for ch in ("_", "*", "`", "["):
...
def escape_md(text: str) -> str:
    """Escape characters that break MarkdownV1 parsing."""
    if not text:
        return ""
    for ch in ["_", "*", "`", "["]:
        text = text.replace(ch, f"\\{ch}")
...
async def set_language(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        t(lang_code, "language_set") + "\n\n" + t(lang_code, "main_menu_text", balance=balance),
...
async def ask_next_field(message: Message, state: FSMContext):
    ...
    if field["type"] == "days":
        buttons = [
            [InlineKeyboardButton(text=opt, callback_data=f"days_{idx}_{opt}")]
            for opt in field["options"]
        ]
        buttons.append([InlineKeyboardButton(text=t(lang, "btn_cancel"), callback_data="main_menu")])
        await message.answer(prompt, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    else:
        await message.answer(prompt, reply_markup=back_button(lang))
...
async def validate_field(field, value: str, service_name: str):
    if field["type"] == FIELD_LINK:
        ...
        if not value.strip().lstrip('-').isdigit():
            return False, None, "❌ Введите числовой ID реакции (например, 12345) или 0 для обычной реакции"
        reaction_id = int(value.strip())
        if reaction_id < 0:
            return False, None, "❌ ID реакции не может быть отрицательным. Для обычной реакции введите 0"
...
async def dynamic_field_text(message: Message, state: FSMContext):
    if message.text and message.text.startswith('/'):
        await state.clear()
        lang = get_lang(message.from_user.id)
        await message.answer(t(lang, "order_cancelled"), reply_markup=main_menu(lang))
        return
    data = await state.get_data()
    idx = data.get("field_index", 0)
    fields = data["schema"]["fields"]
...
async def create_dynamic_order(message: Message, state: FSMContext):
    ...
    if not link or not final_quantity:
        await message.answer(t(lang, "all_fields_required"), reply_markup=main_menu(lang))
        await state.clear()
        return
    ...
    if user["rub_balance"] < total_rub:
        await message.answer(t(lang, "not_enough_funds", balance=user["rub_balance"]), reply_markup=main_menu(lang))
        await state.clear()
        return
    if not deduct_balance_rub(message.from_user.id, total_rub):
        await message.answer(t(lang, "deduct_error"), reply_markup=main_menu(lang))
        await state.clear()
        return
    ...
    if "order" in tw_resp:
        ...
        await message.answer(
            t(lang, "order_success",
              order_id=local_order_id, service=service["name"],
              quantity=final_quantity, total=total_rub),
            reply_markup=main_menu(lang)
        )
    else:
        add_balance_rub(message.from_user.id, total_rub)
        await message.answer(t(lang, "order_error"), reply_markup=main_menu(lang))
    await state.clear()
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    balance = user["rub_balance"]
    await message.answer(
        t(lang, "welcome_back", balance=balance),
        reply_markup=main_menu(lang)
    )
...
async def back_to_main(callback: CallbackQuery, state: FSMContext):
    ...
    balance = user["rub_balance"] if user else 0.0
    await callback.message.edit_text(
        t(lang, "main_menu_text", balance=balance),
        reply_markup=main_menu(lang)
    )
    await callback.answer()
...
async def profile(callback: CallbackQuery, state: FSMContext):
    ...
    text = t(lang, "profile_text",
             ...
             name=safe_md(callback.from_user.full_name),
             balance=balance)
    await callback.message.edit_text(text, parse_mode=ParseMode.MARKDOWN, reply_markup=back_button(lang))
    await callback.answer()
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    if message.text and message.text.startswith('/'):
        await state.clear()
        await message.answer(t(lang, "operation_cancelled"), reply_markup=main_menu(lang))
        return
    if not message.text.isdigit():
        await message.answer(t(lang, "deposit_invalid"))
        return
    stars = int(message.text)
    if stars < 1 or stars > 100000:
        await message.answer(t(lang, "deposit_range"))
        return
    rub_amount = stars * STAR_TO_RUB
    await bot.send_invoice(
        chat_id=message.chat.id,
        title=t(lang, "deposit_invoice_title", stars=stars),
        description=t(lang, "deposit_invoice_desc", rub=rub_amount),
...
async def successful_payment(message: Message, state: FSMContext):
    ...
    if payload.startswith("deposit_"):
        ...
        await message.answer(t(lang, "deposit_success", rub=rub_amount, stars=stars))
    else:
        await process_order_payment(message)
    await message.answer(t(lang, "main_menu_btn"), reply_markup=main_menu(lang))
    ...
    if parts[0] == "order":
        ...
        if "order" in tw_resp:
            ...
            await message.answer(
                t(lang, "order_success",
                  order_id=local_order_id, service=service["name"],
                  quantity=quantity, total=total_rub)
            )
        else:
            add_balance_rub(message.from_user.id, total_rub)
            await message.answer(t(lang, "order_error"))
...
async def show_category(callback: CallbackQuery, state: FSMContext):
    ...
    cat_map = {
        "views":       (t(lang, "cat_views_title"),       views_submenu(lang)),
        "reactions":   (t(lang, "cat_reactions_title"),   reactions_submenu(lang)),
        "subscribers": (t(lang, "cat_subscribers_title"), subscribers_submenu(lang)),
        "boosts":      (t(lang, "cat_boosts_title"),      boosts_submenu(lang)),
        "starts":      (t(lang, "cat_starts_title"),      starts_submenu(lang)),
        "comments":    (t(lang, "cat_comments_title"),    comments_submenu(lang)),
        "polls":       (t(lang, "cat_polls_title"),       polls_submenu(lang)),
        "reposts":     (t(lang, "cat_reposts_title"),     reposts_submenu(lang)),
        "live":        (t(lang, "cat_live_title"),        live_submenu(lang)),
    }
    if cat in cat_map:
        title, markup = cat_map[cat]
        await callback.message.edit_text(title, reply_markup=markup)
    else:
        await callback.message.edit_text(t(lang, "cat_unavailable"), reply_markup=back_button(lang))
    await callback.answer()
...
async def handle_subcategory(callback: CallbackQuery, state: FSMContext):
    ...
    if sub not in SUBCATEGORY_FILTERS:
        await callback.answer(t(lang, "subcat_not_found"))
        return
    ...
    if not filtered:
        await callback.message.edit_text(t(lang, "no_services"), reply_markup=back_button(lang))
        await callback.answer()
        return
    ...
    await send_category_page(callback.message, sub, filtered, 1, state, lang)
    await callback.answer()


async def send_category_page(message, cat, services, page, state, lang="ru"):
    pag = Pagination(services, page_size=8)
    items, current_page, total_pages = pag.get_page(page)
    text = t(lang, "category_page", cat=cat, page=current_page, total=total_pages)
    buttons = []
    for s in items:
    ...
    sort_buttons = [
        InlineKeyboardButton(
            text=t(lang, "price_asc") + (" ✅" if sort_order == "asc" else ""),
            callback_data=f"sort|{cat}|asc"
        ),
        InlineKeyboardButton(
            text=t(lang, "price_desc") + (" ✅" if sort_order == "desc" else ""),
...
async def paginate_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("processing"):
        await callback.answer(t(lang, "wait_processing"))
        return
    ...
    try:
        ...
        if not services:
            await callback.answer("Ошибка")
            return
        await send_category_page(callback.message, cat, services, page, state, lang)
...
async def sort_services(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await send_category_page(callback.message, cat, services_sorted, 1, state, lang)
    except Exception as e:
...
async def service_selected(callback: CallbackQuery, state: FSMContext):
    # Read category_name BEFORE clearing state — it tells us which subcategory
    # the user was browsing, giving us 100% accurate schema lookup regardless
    # of how the API names the service (emoji, price, language variant, etc.)
    ...
    service = next((s for s in services_cache if s["service"] == service_id), None)
    if not service:
        await callback.answer(t(lang, "service_not_found"))
        return
    ...
    # 1st priority: subcategory key — exact match, works for ANY service name
    schema = SUBCATEGORY_TO_SCHEMA.get(category_name)

    # 2nd priority: name-based heuristic (fallback if called outside normal subcategory flow)
    if not schema:
        schema = get_schema_for_service(service_id, service["name"])

    await state.update_data(schema=schema, collected_fields={})
    await start_dynamic_order(callback.message, state, schema, lang)
    await callback.answer()


async def start_dynamic_order(message, state, schema, lang="ru"):
    fields = schema.get("fields", [])
    if not fields:
        await message.answer(t(lang, "no_fields"), reply_markup=main_menu(lang))
        return
    await state.update_data(field_index=0, total_fields=len(fields))
    data = await state.get_data()
    api_description = data.get("api_description", "")

    # BUG FIX: build pre_check text safely — api_description is plain text only,
    # schema["pre_check"] is our own text and may use Markdown
    parts_plain = []
    if api_description:
        parts_plain.append(t(lang, "api_instruction_header") + api_description)

    has_pre_check = "pre_check" in schema
    pre_check_md = schema.get("pre_check", "") if has_pre_check else ""

    if parts_plain or has_pre_check:
        buttons = []
        ...
        buttons.append([InlineKeyboardButton(text=t(lang, "btn_continue"), callback_data="continue_dynamic")])

        # Send api_description as plain text first (if any)
        if parts_plain:
            await message.answer("\n\n".join(parts_plain))

        # Send pre_check with Markdown (it's our own safe text)
        if has_pre_check:
            await message.answer(
                pre_check_md,
                parse_mode=ParseMode.MARKDOWN,
            ...
            # No pre_check — just show the "continue" button
            await message.answer(
                t(lang, "btn_continue"),
                reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
            )
...
async def redirect_to_subscribe(callback: CallbackQuery, state: FSMContext):
    ...
    if not filter_func:
        await callback.message.answer(t(lang, "subcat_not_found"), reply_markup=back_button(lang))
        await callback.answer()
        return
    filtered = [
        s for s in services_cache
    ...
    if not filtered:
        await callback.message.answer(t(lang, "no_services"), reply_markup=back_button(lang))
        await callback.answer()
        return
    filtered.sort(key=lambda s: s.get("_price_rub", float(s["rate"])))
    await state.update_data(category_services=filtered, category_name="reactions_subscribe", page=1, sort_order="asc")
    await send_category_page(callback.message, "reactions_subscribe", filtered, 1, state, lang)
    await callback.answer()
...
async def get_link(message: Message, state: FSMContext):
    ...
    if not link.startswith(("https://t.me/", "http://t.me/")):
        await message.answer(t(lang, "link_must_start"))
        return
    if ("закрытых каналов" in service_name.lower() or "закрытом канале" in service_name.lower()) and "/c/" not in link:
        await message.answer(t(lang, "link_private_format"))
        return
    await state.update_data(link=link)
    await message.answer(t(lang, "enter_quantity"))
...
async def get_quantity(message: Message, state: FSMContext):
    ...
    if message.text and message.text.startswith('/'):
        ...
        return
    if not message.text.isdigit():
        await message.answer(t(lang, "quantity_invalid"))
        return
    quantity = int(message.text)
    data = await state.get_data()
    service = data["service"]
    if quantity < service["min"] or quantity > service["max"]:
        await message.answer(t(lang, "quantity_range", min=service["min"], max=service["max"]))
        return
    ...
    await message.answer(
        t(lang, "order_confirm",
          service=safe_md(service["name"]), link=safe_md(data["link"]),
          quantity=quantity, total=total_rub),
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=confirm_keyboard(service["service"], data["link"], quantity, total_rub, lang)
    )
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    if user["rub_balance"] < total_rub:
        await callback.message.edit_text(
            t(lang, "not_enough_funds", balance=user["rub_balance"]),
            reply_markup=back_button(lang)
        )
        await state.clear()
        await callback.answer()
    ...
    if not deduct_balance_rub(callback.from_user.id, total_rub):
        await callback.message.edit_text(t(lang, "deduct_error"), reply_markup=back_button(lang))
        await state.clear()
        return
    tw_resp = await tw_add_order(service_id, link, quantity)
    if "order" in tw_resp:
        ...
        await callback.message.edit_text(t(lang, "order_error"), reply_markup=main_menu(lang))
    await state.clear()
    await callback.answer()
...
async def status_checker():
    while True:
        ...
        c = conn.cursor()
        c.execute("SELECT order_id, tw_order_id, user_id FROM orders WHERE status = 'processing'")
        rows = c.fetchall()
        conn.close()
        for order_id, tw_order_id, user_id in rows:
            try:
                ...
                if tw_status == "Completed":
                    ...
                    try:
                        await bot.send_message(user_id, t(lang, "order_completed", order_id=order_id))
                    except Exception:
                        pass
                elif tw_status in ("Cancelled", "Canceled"):
                    update_order_status(order_id, "cancelled", str(status_data))
                    try:
                        await bot.send_message(user_id, t(lang, "order_cancelled_status", order_id=order_id))
                    except Exception:
                        pass
            except Exception as e:
                logging.error(f"Status checker error for order {order_id}: {e}")
