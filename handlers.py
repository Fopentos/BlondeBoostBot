...
async def ask_next_field(message: Message, state: FSMContext):
    ...
    if field["type"] == FIELD_SPEED:
        # Скорость фиксирована в ID, но можно показать подсказку
...
async def validate_field(field, value: str, service_name: str) -> tuple[bool, any, str]:
    ...
    if field["type"] == FIELD_LINK:
        ...
        # Дополнительная проверка для закрытых каналов
        if "закрытых каналов" in service_name.lower() or "private" in service_name.lower():
            if "/c/" not in value:
                return False, None, "❌ Для закрытого канала ссылка должна содержать /c/ (пример: https://t.me/c/123456789/301)"
        return True, value, ""
    elif field["type"] == FIELD_QUANTITY:
        if not value.isdigit():
            return False, None, "❌ Введите целое число"
        qty = int(value)
        # Проверка min/max будет позже по данным услуги
        return True, qty, ""
...
async def start_dynamic_order(message, state, schema):
    ...
    if "pre_check" in schema:
        ...
        # Ожидаем нажатия кнопки "Понятно, продолжить"
...
# ---------------------- Админ-панель (уже есть, но для целостности добавим) ----------------------
...
# Остальные админ-обработчики (admin_users, admin_orders, admin_stats, admin_add_balance) уже есть в admin.py,
# но если их нет, их нужно добавить. Для простоты оставим ссылку на admin.py.
# В основном файле admin.py уже есть все необходимые функции.