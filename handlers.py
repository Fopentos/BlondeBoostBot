...
# ---------------------- Фильтры для подкатегорий (окончательные) ----------------------
SUBCATEGORY_FILTERS = {
    # Просмотры
    ...
    # Живые - оставляем только подписчиков
...
async def service_selected(callback: CallbackQuery, state: FSMContext):
    ...
    if schema:
        ...
        if api_description:
            await callback.message.answer(f"📖 *Инструкция от сервиса:*\n{api_description}", parse_mode=ParseMode.MARKDOWN)
        await callback.message.edit_text(
...
async def start_dynamic_order(message, state, schema):
    ...
    final_pre_check = ""
    if api_description:
        final_pre_check += f"📖 *Инструкция от сервиса:*\n{api_description}\n\n"
    if "pre_check" in schema:
        final_pre_check += schema["pre_check"]
    if final_pre_check:
        buttons = []
        ...
        await message.answer(final_pre_check, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
